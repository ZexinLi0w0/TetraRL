#!/usr/bin/env python3
"""Aggregate P15 Phase 3 results (Atari Breakout x Orin AGX, 75 cells).

Reads all summary.json files under runs/p15_phase3_orin_agx_atari/, then emits:
  - _phase3_summary.csv  : long-form, one row per cell (75 rows)
  - _phase3_summary.md   : 21-row x 5-metric table (active (algo,wrapper) pairs),
...
RUNS_DIR = ROOT / "runs" / "p15_phase3_orin_agx_atari"
CSV_PATH = RUNS_DIR / "_phase3_summary.csv"
MD_PATH = RUNS_DIR / "_phase3_summary.md"
...
def write_md(active: list[tuple], skipped: list[tuple]) -> None:
    lines: list[str] = []
    lines.append("# P15 Phase 3 — Atari Breakout × Orin AGX summary")
    lines.append("")
    lines.append(
...
def main() -> None:
    ...
    print(
        f"OK: wrote _phase3_summary.csv ({len(rows)} rows) and _phase3_summary.md"
    )
