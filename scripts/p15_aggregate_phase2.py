#!/usr/bin/env python3
"""Aggregate P15 Phase 2 results (CartPole x Orin Nano, 75 cells).

Reads all summary.json files under runs/p15_phase2_orin_nano_cartpole/, then emits:
  - _phase2_summary.csv  : long-form, one row per cell (75 rows)
  - _phase2_summary.md   : 21-row x 5-metric table (active (algo,wrapper) pairs),
...
RUNS_DIR = ROOT / "runs" / "p15_phase2_orin_nano_cartpole"
CSV_PATH = RUNS_DIR / "_phase2_summary.csv"
MD_PATH = RUNS_DIR / "_phase2_summary.md"
...
def write_md(active: list[tuple], skipped: list[tuple]) -> None:
    lines: list[str] = []
    lines.append("# P15 Phase 2 — CartPole × Orin Nano summary")
    lines.append("")
    lines.append(
...
def main() -> None:
    ...
    print(
        f"OK: wrote _phase2_summary.csv ({len(rows)} rows) and _phase2_summary.md"
    )
